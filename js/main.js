...
const isPriceDropsPage = () => window.location.pathname.includes('/price-drops/');
const isSearchPage = () => window.location.pathname.includes('/search/');
const usesParentRelativeAssets = () => isDealsPage() || isPriceDropsPage() || isSearchPage();

const getProductsIndexUrl = () => usesParentRelativeAssets() ? '../products_index.json' : 'products_index.json';
...
const parseRenderedPrice = (value) => {
    const text = String(value || '');
    const priceMatch = text.match(/₩\s*([\d,]+)/);
    const priceText = priceMatch ? priceMatch[1] : text;
    return parseInt(priceText.replace(/[^\d]/g, ''), 10) || null;
};

const getProductDetailHref = (product) => {
    const detailPath = product.detail_path || `products/product_${product.id}.html`;
    return usesParentRelativeAssets() && !detailPath.startsWith('../') ? `../${detailPath}` : detailPath;
};
...
const hydrateProductsFromDom = () => {
    ...
    return Array.from(productCards).map((card, index) => {
        ...
        const badgeText = card.querySelector('.sale-badge')?.textContent || '';
        const listPriceDropMetaText = card.querySelector('.list-price-drop-meta')?.textContent || '';
        const detailHref = card.querySelector('a[href]')?.getAttribute('href') || '';
        const normalizedDetailPath = detailHref.replace(/^\.\.\//, '');
        const isListPriceDrop = !!card.querySelector('.list-price-drop-badge');
        ...
        return {
            ...
            current_price: parseRenderedPrice(currentPriceText) || 0,
            original_price: parseRenderedPrice(originalPriceText),
            availability: card.querySelector('.stock-status')?.textContent?.trim() || '',
            is_on_sale: !!card.querySelector('.sale-badge'),
            discount_percentage: parseInt(badgeText.match(/\d+/)?.[0] || '0', 10),
            is_list_price_drop: isListPriceDrop,
            list_price_drop_percentage: isListPriceDrop ? parseInt(badgeText.match(/\d+/)?.[0] || '0', 10) : null,
            recent_high_original_price: isListPriceDrop ? parseRenderedPrice(listPriceDropMetaText) : null,
...
const createProductCard = (product) => {
    ...
    const isListPriceDropCard = isPriceDropsPage() || product.is_list_price_drop;
    const listPriceDropPercentage = product.list_price_drop_percentage ?? product.discount_percentage ?? 0;
    
    const saleHtml = isListPriceDropCard
        ? `<div class="sale-badge list-price-drop-badge">정가 ${escapeHtml(listPriceDropPercentage)}%↓</div>`
        : product.is_on_sale
    ...
    const listPriceDropMetaHtml = isListPriceDropCard && product.recent_high_original_price
        ? `<div class="list-price-drop-meta">90일 최고 정가 ₩${formatPrice(product.recent_high_original_price)}</div>`
        : '';
...
const initProductRequest = () => {
    ...
    const setFabScrolling = (isScrolling) => {
        requestProductFab.classList.toggle('is-scrolling', isScrolling);
    };

    const handleProductRequestFabScroll = () => {
        setFabScrolling(true);
        clearTimeout(fabScrollTimer);
        fabScrollTimer = setTimeout(() => setFabScrolling(false), 700);
    };
