    ...
    filterChips.forEach(chip => {
        chip.addEventListener('click', () => {
            ...
            if (category === '전체') {
                ...
                // Here you would filter by actual category
                // For now, we'll do a simple text match
                filteredProducts = allProducts.filter(product => {
                    const text = `${product.name || ''} ${product.full_text || ''}`.toLowerCase();
...
const initProductCards = () => {
    document.addEventListener('click', (e) => {
        ...
        if (card && !e.target.closest('.quick-view-btn')) {
            ...
            window.location.href = `products/product_${productId}.html`;
        }
    });
...
const initGoogleSheetsRequest = () => {
    ...
    const validateUrl = (url) => {
        const pattern = /^https?:\/\/(kr\.)?iherb\.com\/pr\/.+/;
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Request data:', { productUrl: productUrl });
            ...
            // 디버깅 정보
            console.log('Product request submitted (no-cors mode):', productUrl);
...
// Add CSS for list view dynamically
